    ...
    def rollout(self):
        ...
        for epi_rews,epi_values,epi_dones in zip(batch_rewards,batch_values,batch_dones):
            ...
            for t in reversed(range(len(epi_rews))):
                if t+1 < len(epi_rews):
                    delta = epi_rews[t] + self.gamma*epi_values[t+1]*(1-epi_dones[t+1]) - epi_values[t]
                else:
                    delta = epi_rews[t] - epi_values[t]
                advantage = delta + self.gamma*self.lam*(1-epi_dones[t])*last_adv
                last_adv = advantage
